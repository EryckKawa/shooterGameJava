			...
			for (int xx = 0; xx < map.getWidth(); xx++) {
				for (int yy = 0; yy < map.getHeight(); yy++) {
					...
					if (pixelAtual == 0xFF000000) {
						...
						Game.enemies.add(en);
					} else if (pixelAtual == 0xFFFF5B63) {
						// Weapon
						Weapon arma = new Weapon(xx * 16, yy * 16, 16, 16, Entity.WEAPON_EN);
						Game.entities.add(arma);
					} else if (pixelAtual == 0xFF00FF26) {
						// Vida
						Lifepack vida = new Lifepack(xx * 16, yy * 16, 16, 16, Entity.LIFEPACK_EN);
						Game.entities.add(vida);
					} else if (pixelAtual == 0xFFFFD800) {
						Bullet bala = new Bullet(xx * 16, yy * 16, 16, 16, Entity.BULLET_EN);
